function att_show(serverData, file) {
    ...
    $.get('?m=attachment&c=attachments&a=swfupload_json&aid=' + id + '&src=' + src + '&filename=' + filename);
...
function fileQueued(file) {
    if (file != null) {
        try {
            ...
            if (autoStart == 1){
                document.getElementById('btupload').onclick();
            }
        } catch (ex) {
            this.debug(ex);
        }
...
function uploadSuccess(file, serverData) {
    ...
    if (autoStart == 1){
        $('#div_swf_1').append('<p style="margin-top:10px;color:red">文件上传成功，正在返回，请等待1.5秒。</p>');
        setTimeout(function () {
