...
class param
{
    ...
    public function __construct()
    {
        ...
        $this->route_config = extendLoad::load_config('route', SITE_URL) ? extendLoad::load_config('route', SITE_URL) : extendLoad::load_config('route', 'default');
    ...
    public static function set_cookie($var, $value = '', $time = 0)
    {
        ...
        $var = extendLoad::load_config('system', 'cookie_pre') . $var;
        $_COOKIE[$var] = $value;
        if (is_array($value)) {
            foreach ($value as $k => $v) {
                setcookie($var . '[' . $k . ']', sys_auth($v, 'ENCODE', md5(LOAD_PATH . 'cookie' . $var) . extendLoad::load_config('system', 'auth_key')), $time, extendLoad::load_config('system', 'cookie_path'), extendLoad::load_config('system', 'cookie_domain'), $s, $httponly);
            }
        } else {
            setcookie($var, sys_auth($value, 'ENCODE', md5(LOAD_PATH . 'cookie' . $var) . extendLoad::load_config('system', 'auth_key')), $time, extendLoad::load_config('system', 'cookie_path'), extendLoad::load_config('system', 'cookie_domain'), $s, $httponly);
        }
    }
    ...
    {
        $var = extendLoad::load_config('system', 'cookie_pre') . $var;
        $value = isset($_COOKIE[$var]) ? sys_auth($_COOKIE[$var], 'DECODE', md5(LOAD_PATH . 'cookie' . $var) . extendLoad::load_config('system', 'auth_key')) : $default;
