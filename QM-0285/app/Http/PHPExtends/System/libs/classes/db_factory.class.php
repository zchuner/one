...
final class db_factory
{
    ...
    {
        if ($db_config == '') {
            $db_config = extendLoad::load_config('database');
        }
    ...
    public function connect($db_name)
    {
        ...
        switch ($this->db_config[$db_name]['type']) {
            case 'mysql' :
                extendLoad::load_sys_class('mysql', '', 0);
                $object = new mysql();
                break;
            case 'mysqli' :
                extendLoad::load_sys_class('db_mysqli', '', 0);
                $object = new db_mysqli();
                break;
            case 'access' :
                $object = extendLoad::load_sys_class('db_access');
                break;
            default :
                extendLoad::load_sys_class('mysql', '', 0);
                $object = new mysql();
