...
final class template_cache
{
    ...
    public function template_compile($module, $template, $style = 'default')
    {
        if (strpos($module, '/') === false) {
            $tplfile = $_tpl = TEMPLATE_PATH . $style . DIRECTORY_SEPARATOR . $module . DIRECTORY_SEPARATOR . $template . TEMPLATE_PREFIX;
            ...
            $module = str_replace('/', DIRECTORY_SEPARATOR, $module);
            $tplfile = $_tpl = TEMPLATE_PATH . $plugin . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR . $template . TEMPLATE_PREFIX;
        }

        if ($style != 'default' && !file_exists($tplfile)) {
            $style = 'default';
            $tplfile = TEMPLATE_PATH . 'default' . DIRECTORY_SEPARATOR . $module . DIRECTORY_SEPARATOR . $template . TEMPLATE_PREFIX;
        }

        if (!file_exists($tplfile)) {
            exit("模板 " . DIRECTORY_SEPARATOR . $style . DIRECTORY_SEPARATOR . $module . DIRECTORY_SEPARATOR . $template . TEMPLATE_PREFIX . " 不存在！");
        }
    ...
    public function template_parse($str)
    {
        ...
        $str = preg_replace_callback("/\{qm:(\w+)\s+([^}]+)\}/i", [$this, 'qm_tag_callback'], $str);
        $str = preg_replace_callback("/\{\/qm\}/i", [$this, 'end_qm_tag'], $str);
    ...
    public static function qm_tag_callback($matches)
    {
        return self::qm_tag($matches[1], $matches[2], $matches[0]);
    }
     ...
     * @return mixed
     */
    public static function qm_tag($op, $data, $html)
    {
        ...
        if (in_array($op, $tools)) {
            switch ($op) {
                ...
                case 'xml':
                    $str .= '$xml = extendLoad::load_sys_class(\'xml\');';
                    $str .= '$xml_data = @file_get_contents(\'' . $datas['url'] . '\');';
                    $str .= '$' . $return . ' = $xml->xml_unserialize($xml_data);';
                    break;

                case 'get':
                    $str .= 'extendLoad::load_sys_class("get_model", "model", 0);';
                    if ($datas['dbsource']) {
                ...
                case 'block':
                    $str .= '$block_tag = extendLoad::load_app_class(\'block_tag\', \'block\');';
                    $str .= 'echo $block_tag->pc_tag(' . self::arr_to_html($datas) . ');';
                    break;
            }
        } else {
            if (!isset($action) || empty($action)) return false;
            if (module_exists($op) && file_exists(LOAD_PATH . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . $op . DIRECTORY_SEPARATOR . 'classes' . DIRECTORY_SEPARATOR . $op . '_tag.class.php')) {
                $str .= '$' . $op . '_tag = extendLoad::load_app_class("' . $op . '_tag", "' . $op . '");if (method_exists($' . $op . '_tag, \'' . $action . '\')) {';
                if (isset($start) && intval($start)) {
     ...
     * QM 标签结束
     */
    static private function end_qm_tag()
    {
