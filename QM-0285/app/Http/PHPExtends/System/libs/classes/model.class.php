...
extendLoad::load_sys_class('db_factory', '', 0);
...
class model
{
    ...
    {
        $sql = str_replace('qimaweb_', $this->db_tablepre, $sql);
     ...
     *                        为字符串时[例：`name`='qimaweb',`hits`=`hits`+1]。
     *                        为数组时[例: ['name'=>'qimaweb','password'=>'123456']]
     *                        数组的另一种使用 ['name'=>'+=1', 'base'=>'-=1'];程序会自动解析为`name` = `name` + 1, `base` = `base` - 1
