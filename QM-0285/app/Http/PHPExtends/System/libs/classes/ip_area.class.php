...
class ip_area
{
    ...
    {
        if (@file_exists(LOAD_PATH . 'libs' . DIRECTORY_SEPARATOR . 'data' . DIRECTORY_SEPARATOR . 'ipdata' . DIRECTORY_SEPARATOR . 'mini.Dat')) {
            $this->func = 'data_mini';
            $this->fp = @fopen(LOAD_PATH . 'libs' . DIRECTORY_SEPARATOR . 'data' . DIRECTORY_SEPARATOR . 'ipdata' . DIRECTORY_SEPARATOR . 'mini.Dat', 'rb');
            $this->offset = unpack('Nlen', fread($this->fp, 4));
            $this->index = fread($this->fp, $this->offset['len'] - 4);
        } elseif (@file_exists(LOAD_PATH . 'libs' . DIRECTORY_SEPARATOR . 'data' . DIRECTORY_SEPARATOR . 'ipdata' . DIRECTORY_SEPARATOR . 'QQWry.Dat')) {
            $this->func = 'data_full';
            $this->fp = @fopen(LOAD_PATH . 'libs' . DIRECTORY_SEPARATOR . 'data' . DIRECTORY_SEPARATOR . 'ipdata' . DIRECTORY_SEPARATOR . 'QQWry.Dat', 'rb');
        } elseif (@file_exists(LOAD_PATH . 'libs' . DIRECTORY_SEPARATOR . 'data' . DIRECTORY_SEPARATOR . 'ipdata' . DIRECTORY_SEPARATOR . 'qqwry.dat')) {
            $this->func = 'data_full';
            $this->fp = @fopen(LOAD_PATH . 'libs' . DIRECTORY_SEPARATOR . 'data' . DIRECTORY_SEPARATOR . 'ipdata' . DIRECTORY_SEPARATOR . 'qqwry.dat', 'rb');
        }
     ...
     * 通过外部接口方式获取详细地址信息
     * @param string $ip
     * @return string $localinfo province 省份 city城市  sp网络 提供商  pinyin拼音
     */
    public function getcitybyapi($ip)
    {
        $api_url = $localinfo = '';
        $xml = extendLoad::load_sys_class('xml');
