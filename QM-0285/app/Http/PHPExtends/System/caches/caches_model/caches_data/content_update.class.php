...
class content_update
{
    ...
    function editor($field, $value) {
        $attachment_db = extendLoad::load_model('attachment_model');
	...
	function posid($field, $value) {
		if(!empty($value) && is_array($value)) {
			if($_GET['a']=='add') {
				$position_data_db = extendLoad::load_model('position_data_model');
				$textcontent = array();
				foreach($value as $r) {
					if($r!='-1') {
				...
				$push_api = extendLoad::load_app_class('push_api','admin');
				foreach($value as $r) {
					if($r!='-1') $posids[] = $r;
				}
				$textcontent = array();
	...
	function keyword ($field, $value) {
		...
		//加载关键字的数据模型
		$keyword_db = extendLoad::load_model('keyword_model');
		$keyword_data_db = extendLoad::load_model('keyword_data_model');
