...
class attachment_model extends model
{
     ...
     * @isurl intval $isurl 为本地地址时设为1,以cookie形式管理时设置为2
     * @return mixed
     */
    public function api_update($content, $keyid, $isurl = 0)
    {
        if (extendLoad::load_config('system', 'attachment_stat') == 0) return false;
        $keyid = trim($keyid);
        $isurl = intval($isurl);
        if ($isurl == 2 || empty($content)) {
            $this->api_update_cookie($keyid);
        } else {
            $att_index_db = extendLoad::load_model('attachment_index_model');
