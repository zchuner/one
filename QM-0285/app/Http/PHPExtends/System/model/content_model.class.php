...
class content_model extends model
{
    ...
    public function add_content($data, $isimport = 0)
    {
        ...
        //更新没有时长的视频
        if ($this->modelid == 3) $this->updateVideoDuration($this);
    ...
    {
        $this->content_check_db = extendLoad::load_model('content_check_model');
        $this->message_db = extendLoad::load_model('message_model');
     ...
     * @param int $status 状态
     */
    public function delete_content($id, $file, $catid = 0, $status = 0)
    {
        //删除主表数据
    ...
    {
        $this->category_db = extendLoad::load_model('category_model');
        if ($action == 'add') {
     ...
     * @return bool|array
     */
    private function updateVideoDuration($db)
    {
        $noDuration = $db->select("`duration`='0' AND `file_id`!=''");
        foreach ($noDuration as $_k => $_v) {
            $vod = getVodFile($_v['file_id'], 'GetVideoInfo');
            $duration = ($vod['code'] == 0) ? $vod['basicInfo']['duration'] : 0;
            $db->update(['duration' => $duration], ['id' => $_v['id']]);
        }
        return true;
    }
}