...
class block_admin extends admin
{
    ...
    {
        $this->db = extendLoad::load_model('block_model');
        $this->priv_db = extendLoad::load_model('block_priv_model');
        $this->history_db = extendLoad::load_model('block_history_model');
        $this->roleid = session('admin')['roleid'];
    ...
    public function block_update()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if ($data['type'] == 2) {
                ...
                if ($template) {
                    $block = extendLoad::load_app_class('block_tag');
    ...
    public function public_visualization()
    {
        ...
        extendLoad::load_app_func('global', 'template');
        ob_start();
    ...
    public function public_view()
    {
        ...
        if ($data['type'] == 1) {
            ...
            $tpl = extendLoad::load_sys_class('template_cache');
    ...
    public function public_search_content()
    {
        ...
        if (isset($_GET['dosubmit']) && !empty($catid)) {
            ...
            $content_db = extendLoad::load_model('content_model');
