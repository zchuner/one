...
class comment
{
    ...
    {
        $this->comment_db = extendLoad::load_model('comment_model');
        $this->comment_setting_db = extendLoad::load_model('comment_setting_model');
        $this->comment_data_db = extendLoad::load_model('comment_data_model');
        $this->comment_table_db = extendLoad::load_model('comment_table_model');
        $this->comment_check_db = extendLoad::load_model('comment_check_model');
    }
    ...
    public function add($commentid, $siteid, $data, $id = '', $title = '', $url = '')
    {
        ...
        $badword = extendLoad::load_model('badword_model');
        ...
        if ($comment_data_id = $this->comment_data_db->insert($data, true)) {
            ...
            if ($data['status'] == 0) {
                ...
                //extendLoad::load_app_class('receipts', 'pay', 0);
    ...
    public function status($commentid, $id, $status = -1)
    {
        ...
        if ($status == 1) {//通过的时候
            ...
            if (!empty($comment_data['userid']) && !empty($site['del_point']) && module_exists('pay')) {
                //extendLoad::load_app_class('spend', 'pay', 0);
