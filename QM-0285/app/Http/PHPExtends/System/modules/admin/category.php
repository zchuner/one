...
class category extends admin
{
    ...
    {
        parent::__construct();
        $this->db = extendLoad::load_model('category_model');
    ...
    public function init()
    {
        ...
        if (!empty($result)) {
            foreach ($result as $r) {
                ...
                $r['str_manage'] .= '<a href="?m=admin&c=category&a=edit&catid=' . $r['catid'] . '&menuid=' . $_GET['menuid'] . '&type=' . $r['type'] . '">' . L('edit') . '</a> | <a href="javascript:confirmurl(\'?m=admin&c=category&a=delete&catid=' . $r['catid'] . '&menuid=' . $_GET['menuid'] . '\',\'' . L('confirm', array('message' => addslashes($r['catname']))) . '\')">' . L('delete') . '</a> | <a href="?m=admin&c=category&a=remove&catid=' . $r['catid'] . '">' . L('remove', '', 'content') . '</a>';
    ...
    public function add()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            //获取站点模板信息
            extendLoad::load_app_func('global');
    ...
    public function edit()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if ($_POST['template_child']) {
                ...
                if (!empty($idstr)) {
                    $sql = "select catid,setting from qm_category where catid in($idstr)";
                    $this->db->query($sql);
     ...
     * @param int|string $modelid 要删除的栏目模型id
     * @return bool
     */
    ...
    /**
     * 删除栏目分类下的视频
     * @param int $catid 要删除视频的栏目 id
     * @param int|string $modelid 要删除视频的栏目模型 id
     */
    private function delete_category_video($catid, $modelid = '')
    {
        $content_model = extendLoad::load_model('content_model');
    ...
    private function update_url($catid)
    {
        ...
        $url = extendLoad::load_app_class('url', 'content'); //调用URL实例
    ...
    public function batch_edit()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            foreach ($infos as $catid => $info) {
                ...
                //更新附件状态
                if ($info['image'] && extendLoad::load_config('system', 'attachment_stat')) {
