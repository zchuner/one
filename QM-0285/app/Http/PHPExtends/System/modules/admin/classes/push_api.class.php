...
class push_api
{
    ...
    {
        $this->db = extendLoad::load_model('position_model');  //加载数据模型
    }
     ...
     * 调用方式
     * $push = extendLoad::load_app_class('push_api','admin');
    ...
    {
        $array = array();
        $pos_data = extendLoad::load_model('position_data_model');
    ...
    private function content_pos($id, $modelid)
    {
        ...
        $modelid = intval($modelid);
        if ($id && $modelid) {
            $db_data = extendLoad::load_model('position_data_model');
            $this->db_content = extendLoad::load_model('content_model');
    ...
    {
        if ($arr['dosubmit']) {
            if (!$model) $model = 'content_model';
            $db = extendLoad::load_model($model);
