...
class database extends admin
{
    ...
    {
        parent::__construct();
        $this->userid = session('admin')['userid'];
        extendLoad::load_sys_class('db_factory');
    ...
    {
        $database = extendLoad::load_config('database');
    ...
    public function import()
    {
        $database = extendLoad::load_config('database');
        if ($_GET['dosubmit']) {
            $admin_founders = explode(',', extendLoad::load_config('system', 'admin_founders'));
            ...
            if (is_array($sqlfiles)) {
                ...
                foreach ($sqlfiles as $id => $sqlfile) {
                    //老的数据库备份文件转换为新格式
                    if (preg_match("/([qmtables_|db_][0-9]{8}_[0-9a-z]{20}_)([0-9]+)\.sql/i", basename($sqlfile), $num)) {
                    ...
                    if (preg_match("/([0-9a-z]{20}_[qmtables_|db_]+[0-9]{8}_)([0-9]+)\.sql/i", basename($sqlfile), $num)) {
    ...
    private function status($tables, $tablepre)
    {
        ...
        return array('qmtables' => $qm, 'othertables' => $other);
    }

    /**
     * 数据库导出方法
     * @param array $tables 数据表数据组
     * @param string $sqlcompat 数据库兼容类型
     * @param string $sqlcharset 数据库字符
     * @param int $sizelimit 卷大小
     * @param string $action 操作
     * @param string $fileid 卷标
     * @param string $random 随机字段
     * @param string $tableid
     * @param string $startfrom
     * @param string $tabletype 备份数据库类型
     */
    ...
    private function sql_split($sql)
    {
        ...
        if ($this->db_tablepre != "qm_") $sql = str_replace("`qm_", '`' . $this->db_tablepre, $sql);
        $sql = str_replace("\r", "\n", $sql);
        $ret = array();
        $num = 0;
