...
class role extends admin
{
    ...
    {
        parent::__construct();
        $this->db = extendLoad::load_model('admin_role_model');
        $this->priv_db = extendLoad::load_model('admin_role_priv_model');
        $this->op = extendLoad::load_app_class('role_op');
    }
    ...
    public function add()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $this->_cache();
            if ($insert_id) {
                showmessage(L('operation_success'), getExtendsUrl('?c=role&a=init'));
            }
        } else {
    ...
    public function edit()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $this->_cache();
            showmessage(L('operation_success'), getExtendsUrl('?c=role'));
        } else {
    ...
    public function delete()
    {
        ...
        $this->_cache();
        showmessage(L('role_del_success'), getExtendsUrl('?c=role'));
    }
    ...
    {
        $this->menu_db = extendLoad::load_model('menu_model');
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            $roleid = intval($_GET['roleid']);
            if ($siteid) {
                $menu = extendLoad::load_sys_class('tree');
                $menu->icon = array('│ ', '├─ ', '└─ ');
                $menu->nbsp = '&nbsp;&nbsp;&nbsp;';
    ...
    {
        $this->admin_db = extendLoad::load_model('admin_model');
        $roleid = intval($_GET['roleid']);
    ...
    public function setting_cat_priv()
    {
        ...
        switch ($op) {
            case 1:
                ...
                extendLoad::load_app_class('role_cat', '', 0);
