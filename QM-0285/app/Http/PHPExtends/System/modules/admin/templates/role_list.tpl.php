            ...
            if(is_array($infos)){
                foreach($infos as $info){
                    ...
                    <tr>
                        ...
                        <td width="5%"><a href="?c=role&a=change_status&roleid=<?php echo $info['roleid']?>&disabled=<?php echo ($info['disabled']==1 ? 0 : 1)?>"><?php echo $info['disabled']? L('icon_locked'):L('icon_unlock')?></a></td>
                        <td  class="text-c">
                            <?php if($info['roleid'] > 1) {?>
                                <a href="?c=role&a=priv_setting&roleid=<?php echo $info['roleid']?>"><?php echo L('role_setting');?></a> |
                                <a href="?c=role&a=setting_cat_priv&roleid=<?php echo $info['roleid']?>"><?php echo L('usersandmenus')?></a> |
                            <?php } else {?>
                                <font color="#cccccc"><?php echo L('role_setting');?></font> |
                                <font color="#cccccc"><?php echo L('usersandmenus')?></font> |
                            <?php }?>
                            <a href="?c=role&a=member_manage&roleid=<?php echo $info['roleid']?>&menuid=<?php echo $_GET['menuid']?>"><?php echo L('role_member_manage');?></a> |
                            <?php if($info['roleid'] > 1) {?><a href="?c=role&a=edit&roleid=<?php echo $info['roleid']?>&menuid=<?php echo $_GET['menuid']?>"><?php echo L('edit')?></a> |
                                <a href="javascript:confirmurl('?c=role&a=delete&roleid=<?php echo $info['roleid']?>', '<?php echo L('posid_del_cofirm')?>')"><?php echo L('delete')?></a>
                            <?php } else {?>
