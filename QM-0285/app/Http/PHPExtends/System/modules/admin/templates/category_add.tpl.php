...
<form name="myform" id="myform" action="?m=admin&c=category&a=add" method="post">
    <input type="hidden" name="setting[create_to_html_root]" value="0" />
    <input type="hidden" name="info[url]" value="" />
    <div class="pad-10">
        <div class="col-tab">
            <ul class="tabBut cu-li">
                ...
                <li style="display:none" id="tab_setting_6" onclick="SwapTab('setting','on','',6,6);"><?php echo L('catgory_readpoint');?></li>
            </ul>
                        ...
                        <td>
                            ...
                            <span id="batch_add" style="display:none">
                                <table width="100%" class="sss">
                                    <tr>
                                        <td width="310">
                                            <textarea name="batch_add" maxlength="255" style="width:300px;height:60px;"></textarea>
                                        </td>
                                        <td align="left">
            ...
            <div id="div_setting_5" class="contentList pad-10 hidden">
                <table width="100%" >
                    ...
                    <tr><td colspan=2><hr style="border:1px dotted #F2F2F2;"></td></tr>
...
<script type="text/javascript">
    ...
    function change_tpl(modelid) {
        if (modelid) {
            $.getJSON('?m=admin&c=category&a=public_change_tpl&modelid=' + modelid, function (data) {
                $('#template_list').val(data.template_list);
    ...
    function load_file_list(id) {
        if (id == '') return false;
        $.getJSON('?m=admin&c=category&a=public_tpl_file_list&style=' + id + '&catid=<?php echo $parentid?>', function (data) {
            $('#category_template').html(data.category_template);
            $('#list_template').html(data.list_template);
            $('#show_template').html(data.show_template);
        });
