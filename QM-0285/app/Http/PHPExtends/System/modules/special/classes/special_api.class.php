...
class special_api
{
            ...
            if ($a == 'add' && !$v['del']) {
                ...
                if ($siteid > 1) {
                    ...
                    if ($special_info['ishtml']) {
                        ...
                        $url = $site_info['domain'] . 'special/category/' . $specialid . '/' . $typeid . '.html';
                    }
                } else {
                    if ($special_info['ishtml']) $url = addslashes($app_path . extendLoad::load_config('system', 'html_root') . '/special/' . $special_info['filename'] . '/' . $v['typedir'] . '/' . 'type-' . $typeid . '.html');
                ...
                if ((!isset($v['typeid']) || empty($v['typeid'])) && (!isset($v['del']) || empty($v['del']))) {
                    ...
                    if ($siteid > 1) {
                        ...
                        if ($special_info['ishtml']) $url = addslashes($app_path . extendLoad::load_config('system', 'html_root') . '/special/' . $special_info['filename'] . '/' . $v['typedir'] . '/' . 'type-' . $typeid . '.html');
                        else $url = APP_PATH . 'special/category/' . $specialid . '/' . $typeid . '.html';
                    }
                    $v['url'] = $url;
                ...
                if ((!isset($v['del']) || empty($v['del'])) && $v['typeid']) {
                    ...
                    if ($siteid > 1) {
                        ...
                        if ($special_info['ishtml']) {
                            ...
                            $url = $site_info['domain'] . 'special/category/' . $specialid . '/' . $v['typeid'] . '.html';
                        }
                    } else {
                        if ($special_info['ishtml']) $url = addslashes($app_path . extendLoad::load_config('system', 'html_root') . '/special/' . $special_info['filename'] . '/' . $v['typedir'] . '/' . 'type-' . $v['typeid'] . '.html');
                        else $url = APP_PATH . 'special/category/' . $specialid . '/' . $v['typeid'] . '.html';
                    }
                    $v['url'] = $url;
                    $typeid = $v['typeid'];
                    unset($v['typeid']);
    ...
    {
        $c = extendLoad::load_model('content_model');
        if (!$modelid) return '';
        $c->set_model($modelid);
    ...
    public function _del_special($id = 0)
    {
        ...
        if ($info['siteid'] > 1) {
            if ($info['ishtml']) {
                $queue = extendLoad::load_model('queue_model');
                $site = extendLoad::load_app_class('sites', 'admin');
                $site_info = $site->get_by_id($info['siteid']);
                $file = extendLoad::load_config('system', 'html_root') . '/' . $site_info['dirname'] . '/special/' . $info['filename'] . '/index.html';
                if ($info['ispage']) {
                    for ($i = 1; $i > 0; $i++) {
                        if ($i > 1) {
                            $file = str_replace('.html', '-' . $i . '.html', $file);
                        }
                        if (!file_exists(LOAD_PATH . $file)) {
                            break;
                        } else {
                            $queue->add_queue('del', $file, $info['siteid']);
                            unlink(LOAD_PATH . $file);
                        }
                    }
                } else {
                    $queue->add_queue('del', $file, $info['siteid']);
                ...
                $queue->add_queue('del', extendLoad::load_config('system', 'html_root') . '/' . $site_info['dirname'] . '/special/' . $info['filename'] . '/', $info['siteid']);
                dir_delete(extendLoad::load_config('system', 'html_root') . '/' . $site_info['dirname'] . '/special/' . $info['filename']);
            }
        } else {
            if ($info['ishtml']) {
                dir_delete(LOAD_PATH . extendLoad::load_config('system', 'html_root') . DIRECTORY_SEPARATOR . 'special' . DIRECTORY_SEPARATOR . $info['filename']); //删除专题目录
            }
        }
        if (extendLoad::load_config('system', 'attachment_stat')) {
            $keyid = 'special-' . $id;
            $this->attachment_db = extendLoad::load_model('attachment_model');
    ...
    private function delete_type($typeid = 0, $siteid = 0, $ishtml = 0)
    {
        ...
        if ($ishtml) {
            ...
            if ($siteid > 1) {
                ...
                $queue = extendLoad::load_model('queue_model');
                for ($i = 1; $i > 0; $i++) {
                    if ($i == 1) $file = str_replace($site_info['domain'], extendLoad::load_config('system', 'html_root') . '/' . $site_info['dirname'] . '/', $info['url']);
                    else $file = str_replace(array($site_info['domain'], '.html'), array(extendLoad::load_config('system', 'html_root') . '/' . $site_info['dirname'] . '/', '-' . $i . '.html'), $info['url']);
                    if (!file_exists(LOAD_PATH . $file)) {
                        break;
                    } else {
                        $queue->add_queue('del', $file, $siteid); //并加入到消息队列中，便以其他站点删除文件
                        unlink(LOAD_PATH . $file);    //删除生成的静态文件
                    }
                }
            } else {
                for ($i = 1; $i > 0; $i++) {
    ...
    public function _delete_content($cid = 0, $siteid = 0, $ishtml = 0)
    {
        ...
        if ($info['isdata']) {
            if ($ishtml) {
                ...
                if ($siteid > 1) {
                    ...
                    for ($i = 1; $i > 0; $i++) {
                        ...
                        if (!file_exists(LOAD_PATH . $file[1])) {
                            break;
                        } else {
                            $queue->add_queue('del', $file[1], $siteid); //并加入到消息队列中，便以其他站点删除文件
                            unlink(LOAD_PATH . $file[1]);    //删除生成的静态文件
                        }
                    }
                } else {
                    for ($i = 1; $i > 0; $i++) {
            ...
            // 删除数据统计表数据
            $count = extendLoad::load_model('hits_model');
