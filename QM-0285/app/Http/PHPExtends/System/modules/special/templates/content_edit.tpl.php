...
<script type="text/javascript">
    ...
    var uploadurl = '<?php echo UPLOAD_URL?>';
</script>
            ...
            <div class="col-1">
                <div class="content pad-6">
                    ...
                    <div class="upload-pic img-wrap">
                        <div class="bk10"></div>
                        <input type="hidden" name="info[thumb]" value="<?php echo $info['thumb']?>" id="thumb">
                        <a href="javascript:void(0);" onclick="javascript:flashupload('thumb_images', '<?php echo L('file_upload')?>','thumb',thumb_images,'1,jpg|jpeg|gif|bmp|png,300,300','content','39','<?php echo upload_key('1,jpg|jpeg|gif|bmp|png,300,300')?>')">
                            <img src="<?php if($info['thumb']) { echo $info['thumb']; } else {?><?php echo IMG_PATH?>icon/upload-pic.png<?php }?>" id="thumb_preview" width="135" height="113" style="cursor:hand" />
                        </a>
                    </div>
                    <h6> <?php echo L('author')?></h6>
                    <input type="text" name="data[author]" value="<?php echo $data['author']?>" size="30">
                    <h6> <?php echo L('islink')?></h6>
                    <input type="text" name="linkurl" id="linkurl" value="<?php if($info['islink']) { echo $info['url']; }?>" size="30" maxlength="255"<?php if($info['islink']) {?> disabled<?php }?>>
                    <input name="info[islink]" type="checkbox" id="islink" value="1"<?php if($info['islink']) {?> checked<?php }?> onclick="ruselinkurl();" > <font color="red"><?php echo L('islink')?></font>
                    <h6>推荐</h6>
                    <label><input name="info[recommend]" type="radio" value="0"<?php echo ($info['recommend'] == 1) ? '' : ' checked'?> /> 否</label>
                    <label><input name="info[recommend]" type="radio" value="1"<?php echo ($info['recommend'] == 1) ? ' checked' : ''?> /> 是</label>
                ...
                <div class="content pad-6">
                    <table width="100%" cellspacing="0" class="table_form">
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" style="width:350px;" name="info[title]" id="title" value="<?php echo new_html_special_chars($info['title'])?>" class="measure-input " onBlur="$.post('api.php?op=get_keywords&number=3&sid='+Math.random()*5, {data:$('#title').val()}, function(data){if(data && $('#keywords').val()=='') $('#keywords').val(data); })" />
                        ...
                        <tr>
                            <th width="80">视频ID</th>
                            <td>
                                <input type='text' name='info[file_id]' id='file_id' value='<?php echo $info['file_id']?>' readonly>
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <th width="60">
                                <font color="red">*</font> <?php echo L('content')?>
                            </th>
                            <td>
                                <div id='content_tip'></div>
                                <textarea name="data[content]" id="content" boxid="content"><?php echo $data['content']?></textarea><?php echo form::editor('content', 'full', 'content', '', '', 1)?>
                                <div class="content_attr" style="display: none">
                                    <label><input name="add_introduce" type="checkbox"  value="1" checked><?php echo L('iscutcontent')?></label><input type="text" name="introcude_length" value="200" size="3"><?php echo L('characters_to_contents')?>
                        ...
                        <tr>
                            ...
                            <td><select name="data[paginationtype]" id="paginationtype" onchange="if(this.value==1)$('#paginationtype1').css('display','');else $('#paginationtype1').css('display','none');">
                                ...
                                <span id="paginationtype1" style="display:<?php if($data['paginationtype']==1) {?>block<?php } else {?>none<?php }?>">
                                    <input name="data[maxcharperpage]" type="text" id="maxcharperpage" value="<?php echo $data['maxcharperpage']?>" size="8" maxlength="8"><?php echo L('number_of_characters')?>
                                </span>
                            </td>
                        </tr>
                        </tbody>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        }).inputValidator({min: 1, onerror: "<?php echo L('please_choose_type')?>"}).defaultPassed();
        $("#title").formValidator({
            autotip: true,
            onshow: "<?php echo L('please_input_title')?>",
            onfocus: "<?php echo L('please_input_title')?>"
        }).inputValidator({min: 1, onerror: "<?php echo L('please_input_title')?>"}).defaultPassed();
