...
<div class="content">
    <div class="formbody">
        <div class="usual">
            <div class="itab">
                <ul>
                    <li><a id="tab_setting_1" class="selected" onclick="SwapTab('setting','selected','',6,1);"><?php echo L('catgory_basic', '', 'admin');?></a></li>
                    <li><a id="tab_setting_2" onclick="SwapTab('setting','selected','',6,2);"><?php echo L('extend_setting')?></a></li>
                </ul>
            </div>
            <form method="post" action="?m=special&c=special&a=add" id="myform">
                <input type="hidden" name="special[ishtml]" value="0">
                <div id="div_setting_1" class="tabson">
                    <table width="100%" class="table_form ">
                        <tbody>
                        <tr>
                            <th width="100"><?php echo L('special_title')?>：</th>
                        ...
                        <tr>
                            <th>
                                <?php echo L('special_type')?>：<a href="javascript:addItem()" title="<?php echo L('add')?>"><span style="color:red;" >+</span></a>
                            </th>
                            <td valign="top">
                                <div id="option_list">
                                    <div class="mb6">
                                        <span>
                                            <?php echo L('type_name')?>：<input type="text" id="type_name" name="type[1][name]" class="input-text" size="15" style="width:auto">&nbsp;&nbsp;
                                            <?php echo L('type_path')?>：<input type="text" name="type[1][typedir]" id="type_path" class="input-text" size="15" style="width:auto">&nbsp;&nbsp;
                                            <?php echo L('listorder')?>：<input type="text" name="type[<?php echo $k?>][listorder]" value="1" size="6" class="input-text" style="width:auto">
                                        </span>&nbsp;
                    ...
                    <table width="100%" class="table_form ">
                        <tr>
                            ...
                            <td>
                                ...
                                <input type="button" class="button" value="<?php echo L('choose_pic_news')?>" onclick="import_info('?m=special&c=special&a=public_get_pics','<?php echo L('choose_pic_news')?>', 'msg_id', 'relation', 'pics');">
                                <input type="hidden" name="special[pics]" value="" id="pics">
                                <span class="onShow">(<?php echo L('choose_pic_model')?>)</span>
                            </td>
                        </tr>
                        <tr>
...
<script type="text/javascript">
    ...
    function import_info(url, title, msgID, htmlID, valID) {
        window.top.art.dialog({id: 'selectid', iframe: "<?php echo getExtendsUrl()?>" + url, title: title, width: '600', height: '400', lock: true},
            function () {
    ...
    function addItem() {
        ...
        var newOption = '<div class="mb6"><span>' +
            '<?php echo L('type_name')?>：<input type="text" name="type[' + n + '][name]" class="input-text" size="15" style="width:auto">&nbsp;&nbsp;&nbsp;' +
            '<?php echo L('type_path')?>：<input type="text" name="type[' + n + '][typedir]" class="input-text" size="15" style="width:auto">&nbsp;&nbsp;&nbsp;' +
            '<?php echo L('listorder')?>：<input type="text" name="type[' + n + '][listorder]" value="' + n + '" size="6" class="input-text" style="width:auto">&nbsp;&nbsp;&nbsp;' +
            '</span>&nbsp;' +
            '<a href="javascript:;" onclick="descItem(this, ' + n + ');"><?php echo L('remove')?></a>' +
            '</div>';
    ...
    $(document).ready(function () {
        ...
        $('#title').formValidator({
            ...
            type: "get",
            url: "",
            data: "m=special&c=special&a=public_check_special",
            datatype: "html",
            cached: false,
            async: 'true',
        ...
        $('#banner').formValidator({
            autotip: true,
            onshow: "<?php echo L('please_upload_banner')?>",
            oncorrect: "<?php echo L('true')?>"
        }).inputValidator({min: 1, onerror: "<?php echo L('please_upload_banner')?>"});
        $('#thumb').formValidator({
            autotip: true,
            onshow: "<?php echo L('please_upload_thumb')?>",
            oncorrect: "<?php echo L('true')?>"
        }).inputValidator({min: 1, onerror: "<?php echo L('please_upload_thumb')?>"});
        ...
        $('#type_path').formValidator({
            ...
            oncorrect: "<?php echo L('true')?>"
        }).inputValidator({min: 2, onerror: "<?php echo L('input_type_path')?>"}).regexValidator({
            regexp: "^\\w*$",
