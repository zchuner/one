...
class special extends admin
{
    ...
    {
        parent::__construct();
        $this->db = extendLoad::load_model('special_model');
        $this->special_api = extendLoad::load_app_class('special_api', 'special');
    }
            ...
            if ($id) {
                ...
                if ($special['siteid'] > 1) {
                    ...
                    if ($special['ishtml']) {
                        ...
                        $url = $site_info['domain'] . 'special/' . $id . '.html';
                    }
                } else {
                    $url = $special['ishtml'] ? APP_PATH . substr(extendLoad::load_config('system', 'html_root'), 1) . '/special/' . $special['filename'] . '/' : APP_PATH . 'special/' . $id . '.html';
                }
                ...
                //调用生成静态类
                if ($special['ishtml']) {
                    $html = extendLoad::load_app_class('html', 'special');
                    $html->_index($id, 20, 5);
                }
                //更新附件状态
        ...
        if (isset($_POST['dosubmit']) && !empty($_POST['dosubmit'])) {
            ...
            if ($special['ishtml'] && $special['filename']) {
                ...
                if ($siteid > 1) {
                    ...
                    $special['url'] = APP_PATH . substr(extendLoad::load_config('system', 'html_root'), 1) . '/special/' . $special['filename'] . '/';
                }
            } elseif ($special['ishtml'] == '0') {
                if ($siteid > 1) {
                    $special['url'] = $site_info['domain'] . 'special/' . $_GET['specialid'] . '.html';
                } else {
                    $special['url'] = APP_PATH . 'special/' . $_GET['specialid'] . '.html';
                }
            }
    ...
    {
        $siteid = get_siteid();
        $html = extendLoad::load_app_class('html');
        $c = extendLoad::load_model('special_model');
    ...
    public function public_create_type()
    {
        ...
        if (is_array($types) && !empty($types) || $pages) {
            ...
            if (!$pages) {
                $c = extendLoad::load_model('special_content_model');
            ...
            $special_api = extendLoad::load_app_class('special_api');
    ...
    public function public_check_special()
    {
        ...
        if (extendLoad::load_config('system', 'charset') == 'gbk') {
    ...
    private function check($data, $a = 'add')
    {
        ...
        if ($data['voteid']) {
            if (strpos($data['voteid'], '|') === false) {
                $vote_db = extendLoad::load_model('vote_subject_model');
