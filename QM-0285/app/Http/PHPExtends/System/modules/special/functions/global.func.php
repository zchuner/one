 ...
 * @param int $page 当前页
 * @param int $addtime 文章发布时间
 * @param string $type 返回路径的格式（.html|.php）
 * @param array $site_info 站点信息
 * @param string $type 类型 静态地址 $type = 'html', 动态地址 $type='php'
 * @return mixed
 */
function content_url($contentid = 0, $page = 1, $addtime, $type = 'html', $site_info = '')
{
    ...
    switch ($type) {
        case 'html':
            if ($site_info['dirname']) {
                if ($page == 1) {
                    ...
                    $url[1] = extendLoad::load_config('system', 'html_root') . '/' . $site_info['dirname'] . '/special/' . date('Y', $addtime) . '/' . date('md', $addtime) . '/' . $contentid . '.html';
                } else {
                    $url[0] = $site_info['domain'] . 'special/' . date('Y', $addtime) . '/' . date('md', $addtime) . '/' . $contentid . '-' . $page . '.html';
                    $url[1] = extendLoad::load_config('system', 'html_root') . '/' . $site_info['dirname'] . '/special/' . date('Y', $addtime) . '/' . date('md', $addtime) . '/' . $contentid . '-' . $page . '.html';
                }
            } else {
                if ($page == 1) {
                    $url[0] = $url[1] = extendLoad::load_config('system', 'html_root') . '/special/' . date('Y', $addtime) . '/' . date('md', $addtime) . '/' . $contentid . '.html';
                    $url[0] = $app_path . $url[0];
                } else {
                    $url[0] = $url[1] = extendLoad::load_config('system', 'html_root') . '/special/' . date('Y', $addtime) . '/' . date('md', $addtime) . '/' . $contentid . '-' . $page . '.html';
                    $url[0] = $app_path . $url[0];
                }
            }
            break;
        ...
        case 'php':
            if ($page == 1) {
                $url[0] = APP_PATH . 'special/show/' . $contentid . '/0.html';
            } else {
                $url[0] = APP_PATH . 'special/show/' . $contentid . '/' . $page . '.html';
            }
            break;
    }
    return $url;
