...
if (param::get_cookie('sys_lang')) define('SYS_STYLE', param::get_cookie('sys_lang'));
else define('SYS_STYLE', 'zh-cn');

class attachments
{
    private $att_db, $upload_url, $upload_path, $imgext, $groupid, $admin_username;
    ...
    function __construct()
    {
        ...
        $this->userid = session('admin')['userid'] ? session('admin')['userid'] : (param::get_cookie('_userid') ? param::get_cookie('_userid') : sys_auth($_POST['userid_flash'], 'DECODE'));
        $this->isadmin = $this->admin_username = session('admin')['roleid'] ? 1 : 0;
    ...
    public function upload()
    {
        ...
        extendLoad::load_sys_class('attachment', '', 0);
    ...
    public function swfupload()
    {
        ...
        if (isset($_POST['dosubmit']) && $_POST['PHPSESSID']) {

            if ($_POST['swf_auth_key'] != md5(extendLoad::load_config('system', 'auth_key') . $_POST['SWFUPLOADSESSID']) || ($_POST['isadmin'] == 0 && !$grouplist[$_POST['groupid']]['allowattachment'])) exit(0);
            extendLoad::load_sys_class('attachment', '', 0);
    ...
    public function crop_upload()
    {
        if (isset($GLOBALS["HTTP_RAW_POST_DATA"])) {
            ...
            if (isset($_GET['file']) && !empty($_GET['file'])) {
                ...
                if (strpos($_GET['file'], UPLOAD_URL) !== false) {
                    $file = $_GET['file'];
            ...
            exit(UPLOAD_URL . $filepath . $new_file);
        }
    ...
    {
        $attachment = extendLoad::load_sys_class('attachment');
    ...
    public function album_load()
    {
        ...
        $where = $uploadtime = '';
        $this->att_db = extendLoad::load_model('attachment_model');
        ...
        extendLoad::load_sys_class('form');
    ...
    public function album_dir()
    {
        ...
        $local = str_replace(array(LOAD_PATH, ROOT_PATH, DIRECTORY_SEPARATOR . DIRECTORY_SEPARATOR), array('', '', DIRECTORY_SEPARATOR), $filepath);
    ...
    final public static function admin_tpl($file, $m = '')
    {
        ...
        return LOAD_PATH . 'modules' . DIRECTORY_SEPARATOR . $m . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR . $file . '.tpl.php';
    }
