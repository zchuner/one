...
function file_icon($file, $type = 'png')
{
    ...
    if (in_array($ext, $ext_arr)) return IMG_PATH . 'ext/' . $ext . '.' . $type;
    else return IMG_PATH . 'ext/blank.' . $type;
}
...
function file_list($dirpath, $currentdir)
{
    ...
    $list['local'] = str_replace(array(LOAD_PATH, DIRECTORY_SEPARATOR . DIRECTORY_SEPARATOR), array('', DIRECTORY_SEPARATOR), $filepath);
    return $list;
}
 ...
 * @param string $isadmin 是否为管理员模式
 * @param string $userid_flash 是否为管理员模式
 * @return mixed
 */
function initupload($module, $catid, $args, $userid, $groupid = '8', $isadmin = '0', $userid_flash = '0')
{
    ...
    $sess_id = SYS_TIME;
    $admin_url = '';
    $swf_auth_key = md5(extendLoad::load_config('system', 'auth_key') . $sess_id);
    $init = 'var swfu = \'\';
		$(document).ready(function(){
            swfu = new SWFUpload({
                flash_url:"' . JS_PATH . 'swfupload/swfupload.swf?" + Math.random(),
                upload_url: "' . url('admin/extend?m=attachment&c=attachments&a=swfupload&dosubmit=1') . '",
                file_post_name : "Filedata",
                post_params:{"_token":"' . csrf_token() . '","SWFUPLOADSESSID":"' . $sess_id . '","module":"' . $module . '","catid":"' . $_GET['catid'] . '","userid":"' . $userid . '","siteid":"' . $siteid . '","dosubmit":"1","thumb_width":"' . $thumb_width . '","thumb_height":"' . $thumb_height . '","watermark_enable":"' . $watermark_enable . '","filetype_post":"' . $file_types_post . '","swf_auth_key":"' . $swf_auth_key . '","isadmin":"' . $isadmin . '","groupid":"' . $groupid . '","userid_flash":"' . $userid_flash . '", "PHPSESSID": "' . session()->getId() . '"},
