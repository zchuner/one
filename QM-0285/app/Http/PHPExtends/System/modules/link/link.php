...
class link extends admin
{
    function __construct()
    {
        ...
        $this->db = extendLoad::load_model('link_model');
        $this->db2 = extendLoad::load_model('type_model');
    }
    ...
    {
        $link_title = isset($_GET['link_name']) && trim($_GET['link_name']) ? (extendLoad::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['link_name'])) : trim($_GET['link_name'])) : exit('0');
    ...
    {
        $type_name = isset($_GET['type_name']) && trim($_GET['type_name']) ? (extendLoad::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['type_name'])) : trim($_GET['type_name'])) : exit('0');
    ...
    public function add()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            //更新附件状态
            if (extendLoad::load_config('system', 'attachment_stat') & $_POST['link']['logo']) {
    ...
    public function setting()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            //更新模型数据库,重设setting 数据.
            $m_db = extendLoad::load_model('module_model'); //调用模块数据模型
