...
class content extends admin
{
    ...
    {
        parent::__construct();
        $this->db = extendLoad::load_model('content_model');
        $this->siteid = $this->get_siteid();
        $this->categorys = getcache('category_content_' . $this->siteid, 'commons');
        //权限判断
        if (isset($_GET['catid']) && session('admin')['roleid'] != 1 && ROUTE_A != 'pass' && strpos(ROUTE_A, 'public_') === false) {
            $catid = intval($_GET['catid']);
            $this->priv_db = extendLoad::load_model('category_priv_model');
    ...
    public function init()
    {
        ...
        if (isset($_GET['catid']) && $_GET['catid'] && $this->categorys[$_GET['catid']]['siteid'] == $this->siteid) {
            ...
            //工作流权限判断
            if (session('admin')['roleid'] != 1 && $steps && !in_array($steps, $admin_privs)) showmessage(L('permission_to_operate'));
            ...
            if ($category['child']) {
                $catids_str = $category['arrchildid'];
                $where = "catid  IN ($catids_str) AND status=$status";
            } else {
            ...
            for ($i = 1; $i <= $workflow_steps; $i++) {
                if (session('admin')['roleid'] != 1 && !in_array($i, $admin_privs)) continue;
        ...
        if (isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
            ...
            if ($category['type'] == 0) {
                ...
                if ($workflowid && $_POST['status'] != 99) {
                    ...
                    $_POST['info']['status'] = session('admin')['roleid'] == 1 ? intval($_POST['status']) : 1;
                } else {
                ...
                if (isset($_POST['dosubmit'])) {
                    showmessage(L('add_success'), '?m=content&c=content&a=init&catid=' . $catid);
                } else {
                ...
                //单网页
                $this->page_db = extendLoad::load_model('page_model');
            ...
            if (isset($_GET['catid']) && $_GET['catid']) {
                ...
                if ($category['type'] == 0) {
                    ...
                    $admin_username = param::get_cookie('admin_username');
                    $priv_status = (!empty($nocheck_users) && in_array($admin_username, $nocheck_users)) ? true : false;

                    $model_arr = getcache('model', 'commons');
                    $MODEL = $model_arr[$modelid];

                    $template = LOAD_PATH . 'modules' . DIRECTORY_SEPARATOR . 'content' . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR . $MODEL['admin_add_template'] . '.tpl.php';
                    $template = (file_exists($template)) ? $MODEL['admin_add_template'] : 'content_add';
                    include $this->admin_tpl($template);
                } else {
                    //单网页
                    $this->page_db = extendLoad::load_model('page_model');
    ...
    public function edit()
    {
        ...
        if (isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
            ...
            if (isset($_POST['dosubmit'])) {
                showmessage(L('update_success'), '?m=content&c=content&a=init&catid=' . $catid);
            } else {
            ...
            $template = LOAD_PATH . 'modules' . DIRECTORY_SEPARATOR . 'content' . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR . $MODEL['admin_add_template'] . '.tpl.php';
            $template = (file_exists($template)) ? $MODEL['admin_edit_template'] : 'content_edit';
            include $this->admin_tpl($template);
        }
        header("Cache-control: private");
    }
    ...
    public function delete()
    {
        if (isset($_GET['dosubmit'])) {
            ...
            $html_root = extendLoad::load_config('system', 'html_root');
            if ($sethtml) $html_root = '';

            $setting = string2array($this->categorys[$catid]['setting']);
            $content_ishtml = $setting['content_ishtml'];
            $this->db->set_model($modelid);
            $this->hits_db = extendLoad::load_model('hits_model');
            ...
            //附件初始化
            $attachment = extendLoad::load_model('attachment_model');
            $this->content_check_db = extendLoad::load_model('content_check_model');
            $this->position_data_db = extendLoad::load_model('position_data_model');
            $this->search_db = extendLoad::load_model('search_model');
            //判断视频模块是否安装
            if (module_exists('video') && file_exists(LOAD_PATH . 'model' . DIRECTORY_SEPARATOR . 'video_content_model.class.php')) {
                $video_content_db = extendLoad::load_model('video_content_model');
                $video_install = 1;
            }

            $this->comment = extendLoad::load_app_class('comment', 'comment');
            $search_model = getcache('search_model_' . $this->siteid, 'search');
            $typeid = $search_model[$modelid]['typeid'];
            $this->url = extendLoad::load_app_class('url', 'content');
            ...
            foreach ($_POST['ids'] as $id) {
                ...
                //删除内容
                $this->db->delete_content($id, $fileurl, $catid, $r['status'], 0);
    ...
    public function pass()
    {
        ...
        if ($workflowid) {
            ...
            $modelid = $this->categorys[$catid]['modelid'];
            $this->db->set_model($modelid);
            $this->db->search_db = extendLoad::load_model('search_model');
            //审核通过，检查投稿奖励或扣除积分
            if ($status == 99) {
                $html = extendLoad::load_app_class('html', 'content');
                $this->url = extendLoad::load_app_class('url', 'content');
    ...
    public function public_categorys()
    {
        ...
        if ($from == 'content' && session('admin')['roleid'] != 1) {
            $this->priv_db = extendLoad::load_model('category_priv_model');
            $priv_result = $this->priv_db->select(array('action' => 'init', 'roleid' => session('admin')['roleid'], 'siteid' => $this->siteid, 'is_admin' => 1));
            $priv_catids = array();
        ...
        if (!empty($this->categorys)) {
            foreach ($this->categorys as $r) {
                ...
                if ($from == 'content' && session('admin')['roleid'] != 1 && !in_array($r['catid'], $priv_catids)) {
                ...
                if ($r['type'] == 1 || $from == 'block') {
                    if ($r['type'] == 0) {
                        $r['vs_show'] = "<a target='right' href='?m=block&c=block_admin&a=public_visualization&menuid=" . $_GET['menuid'] . "&catid=" . $r['catid'] . "&type=show'>[" . L('content_page') . "]</a>";
                    } else {
                        $r['vs_show'] = '';
                    }
                    $r['icon_type'] = 'file';
                    ...
                    $r['type'] = 'init';
                    $r['add_icon'] = "<a target='right' href='?m=content&c=content&menuid=" . $_GET['menuid'] . "&catid=" . $r['catid'] . "'><img src='" . IMG_PATH . "add_content.gif' alt='" . L('add') . "'></a> ";
                }
                $categorys[$r['catid']] = $r;
            }
        }
        if (!empty($categorys)) {
            ...
            switch ($from) {
                ...
                default:
                    ...
                    $strs2 = "<a href='?m=content&c=content&a=\$type&menuid=" . $_GET['menuid'] . "&catid=\$catid' target='right' onclick='open_list(this)' class='folder'>\$catname</a>";
                    break;
            }
    ...
    public function public_preview()
    {
        ...
        if ($rs['paginationtype'] == 1) {
            ...
            $contentpage = extendLoad::load_app_class('contentpage');
    ...
    public function public_checkall()
    {
        ...
        $pagesize = 20;
        $sql = '';
        if (in_array(session('admin')['roleid'], array('1'))) {
            $super_admin = 1;
        ...
        if ($status != 0 && !$super_admin) {
            ...
            foreach ($this->categorys as $catid => $cat) {
                ...
                //查看管理员是否有这个栏目的查看权限。
                if (!$this->priv_db->get_one(array('catid' => $catid, 'siteid' => $this->siteid, 'roleid' => session('admin')['roleid'], 'is_admin' => '1'))) {
                    continue;
                }
                //如果栏目有设置工作流，进行权限检查。
                $workflow = array();
    ...
    public function public_getsite_categorys()
    {
        ...
        if (session('admin')['roleid'] != 1) {
            $this->priv_db = extendLoad::load_model('category_priv_model');
            $priv_result = $this->priv_db->select(array('action' => 'add', 'roleid' => session('admin')['roleid'], 'siteid' => $siteid, 'is_admin' => 1));
            $priv_catids = array();
            foreach ($priv_result as $_v) {
                $priv_catids[] = $_v['catid'];
            }
        ...
        foreach ($this->categorys as $r) {
            ...
            if (session('admin')['roleid'] != 1 && !in_array($r['catid'], $priv_catids)) {
                $arrchildid = explode(',', $r['arrchildid']);
                $array_intersect = array_intersect($priv_catids, $arrchildid);
                if (empty($array_intersect)) continue;
            }
            ...
            $r['style'] = '';
            $r['click'] = "onclick=\"select_list(this,'" . safe_replace($r['catname']) . "'," . $r['catid'] . ")\" class='cu' title='" . L('click_to_select') . "'";
            $categorys[$r['catid']] = $r;
        }
        $str = "<tr \$click >
                        ...
                        if ($t == 'content') {
                            ...
                            foreach ($model as $modelid) {
                                ...
                                if ($r = $db->count()) { //判断模型下是否有数据
                                    ...
                                    $db->query('TRUNCATE TABLE `qm_' . $model_arr[$modelid]['tablename'] . '`');
                                    ...
                                    $db->query('TRUNCATE TABLE `qm_' . $model_arr[$modelid]['tablename'] . '_data`');

                                    //删除该模型中在hits表的数据
                                    $hits_db = extendLoad::load_model('hits_model');
                                    $hitsid = 'c-' . $modelid . '-';
                                    ...
                                    //删除该模型在search中的数据
                                    $search_db = extendLoad::load_model('search_model');
                                    ...
                                    $position_db = extendLoad::load_model('position_data_model');
                                    ...
                                    //附件初始化
                                    //$attachment = extendLoad::load_model('attachment_model');
                                    //$comment = extendLoad::load_app_class('comment', 'comment');
                                    //if(module_exists('comment')){
                                    //$comment_exists = 1;
                                    //}
                                    //foreach ($data as $d) {
                            ...
                            $db = extendLoad::load_model($t . '_model');
                            if ($r = $db->count()) {
                                $result = $db->select();
    ...
    /**
     * 备份数据到文件
     * @param array $data array 备份的数据数组
