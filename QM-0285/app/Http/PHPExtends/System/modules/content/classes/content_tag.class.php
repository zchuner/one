...
{
    private $db;
    public $modelid, $category;

    public function __construct()
    {
        $this->db = extendLoad::load_model('content_model');
        $this->position = extendLoad::load_model('position_data_model');
        $this->hits_model_db = extendLoad::load_model('hits_model');
        $this->wap_site = getcache('wap_site', 'wap');
        $this->wap = $this->wap_site[1];
    }
    ...
    public function lists($data)
    {
        ...
        if ($data['mobile'] == 1 && $this->wap['status'] == 1) {
            foreach ($return as $_k => $_v) {
                //如果是手机端请求，那么将url转换为手机站绑定的域名
                $return[$_k]['url'] = APP_PATH . 'index.php?m=wap&a=show&catid=' . $_v['catid'] . '&id=' . $_v['id'];
            }
        }
        ...
        foreach ($return as $_k => $_v) {
            $modelid = $CAT['modelid'];
            $_views = $this->hits_model_db->get_one(array('hitsid' => 'c-' . $modelid . '-' . $_v['id']));
            $return[$_k]['views'] = $_views['views'];
        }
    ...
    public function category($data)
    {
        ...
        if ($data['mobile'] == 1 && $this->wap['status'] == 1) {
            foreach ($array as $_k => $_v) {
                //如果是手机端请求，那么将url转换为手机站绑定的域名
                $array[$_k]['url'] = ($_v['type'] != 2) ? APP_PATH . 'index.php?m=wap&a=lists&catid=' . $_v['catid'] : $_v['url'];
            }
        }
    ...
    public function position($data)
    {
        ...
        if (!empty($pos_arr)) {
            ...
            $show_db = extendLoad::load_model('content_model');;
            foreach ($pos_arr as $info) {
                $modelId = $category[$info['catid']]['modelid'];
                $show_db->set_model($modelId);
                $show = $show_db->get_one(['id' => $info['id']], "`status`");
                if ($show['status'] == 99) {
        ...
        if ($data['mobile'] == 1 && $this->wap['status'] == 1) {
            foreach ($array as $_k => $_v) {
                //如果是手机端请求，那么将url转换为手机站绑定的域名
                $array[$_k]['url'] = APP_PATH . 'index.php?m=wap&a=show&catid=' . $_v['catid'] . '&id=' . $_v['id'];
            }
        }
    ...
    public function pc_tag()
    {
        ...
        return [
            ...
            'lists' => [
                'catid' => [
                    'name' => L('catid', '', 'content'),
                    'htmltype' => 'input_select_category',
                    'data' => [
                        'type' => 0
                    ],
                    'validator' => [
                ...
                'order' => [
                    ...
                    'htmltype' => 'select',
                    'data' => [
                        'id DESC' => L('id_desc', '', 'content'),
                        'updatetime DESC' => L('updatetime_desc', '', 'content'),
                        'listorder ASC' => L('listorder_asc', '', 'content')
                    ]
                ],
                'thumb' => [
                    'name' => L('thumb', '', 'content'),
                    'htmltype' => 'radio',
                    'data' => [
                        '0' => L('all_list', '', 'content'),
                        '1' => L('thumb_list', '', 'content')
                    ]
                ],
                'moreinfo' => [
                    'name' => L('moreinfo', '', 'content'),
                    'htmltype' => 'radio',
                    'data' => [
                        '1' => L('yes'),
                        '0' => L('no')
            ...
            'position' => [
                ...
                'order' => [
                    ...
                    'htmltype' => 'select',
                    'data' => [
                        'listorder DESC' => L('listorder_desc', '', 'content'),
                        'listorder ASC' => L('listorder_asc', '', 'content'),
                        'id DESC' => L('id_desc', '', 'content')
                    ]
                ],
            ...
            'relation' => [
                ...
                'relation' => [
                    'name' => L('relevant_articles_id', '', 'content'),
                    'htmltype' => 'input'
                ],
                'keywords' => [
                    'name' => L('key_word', '', 'content'),
                    'htmltype' => 'input'
                ]
            ],
            'hits' => [
