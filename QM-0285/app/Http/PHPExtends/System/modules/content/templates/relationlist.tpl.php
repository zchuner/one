...
<SCRIPT LANGUAGE="JavaScript">
    function select_list(obj, title, id) {
        var relation_ids = $(window.parent.document.body).find('#relation').val();
        var sid = 'v<?php echo $modelid;?>' + id;
        if ($(obj).attr('class') == 'line_ff9966' || $(obj).attr('class') == null) {
            $(obj).attr('class', 'line_fbffe4');
            $(window.parent.document.body).find('#' + sid).remove();
            if (relation_ids != '' && relation_ids != undefined) {
                ...
                $(window.parent.document.body).find('#relation').val(newrelation_ids);
            }
        } else {
            ...
            $(window.parent.document.body).find('#relation_text').append(str);
            if (relation_ids == '') {
                $(window.parent.document.body).find('#relation').val(id);
            } else {
                relation_ids = relation_ids + '|' + id;
                $(window.parent.document.body).find('#relation').val(relation_ids);
            }
        }
    }
</SCRIPT>
