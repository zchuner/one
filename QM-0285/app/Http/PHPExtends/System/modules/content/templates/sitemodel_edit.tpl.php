...
<div class="pad-lr-10">
    <form action="?m=content&c=sitemodel&a=edit" method="post" id="myform">
        ...
        <fieldset>
            <legend>
                <?php echo L('other_template_setting')?>
                <input type="checkbox"<?php if($admin_list_template){?> checked<?php }?> id="other" value="1" name="other">
            </legend>
            <table width="100%" id="other_tab" class="table_form" style="display:none;">
                <?php foreach ($admin_content_template as $k => $v) {?>
                <tr>
                    <th width="200"><?php echo L('admin_content_' . $k)?></th>
                    <td  id="admin_list_template">
                        <?php echo $v;?>
                    </td>
                </tr>
                <?php }?>
                <tr style="display:none">
                    ...
                    <td  id="member_add_template">
                        <?php echo form::select_template($default_style,'member', $member_add_template, 'name="setting[member_add_template]" id="template_member_add"', 'content_publish')?>
                    </td>
                </tr>
            </table>
        </fieldset>
