...
<div class="pad-10">
    <div class="content-menu ib-a blue line-x">
        <a class="add fb" href="?m=content&c=content&a=add&menuid=&catid=<?php echo $catid;?>"><em><?php echo L('add_content');?></em></a>　
        <a href="?m=content&c=content&a=init&catid=<?php echo $catid;?>" <?php if($steps==0 && !isset($_GET['reject'])) echo 'class=on';?>><em><?php echo L('check_passed');?></em></a><span>|</span>
        ...
        <?php if($category['ishtml']) {?>
            <span>|</span><a href="?m=content&c=create_html&a=category&pagesize=30&dosubmit=1&modelid=0&catids[0]=<?php echo $catid;?>&referer=<?php echo urlencode($_SERVER['QUERY_STRING']);?>"><em><?php echo L('update_htmls',array('catname'=>$category['catname']));?></em></a>
        <?php }?>
    </div>
        ...
        <div class="table-list">
            <table width="100%">
                ...
                <tr>
                    ...
                    <th width="118"><?php echo L('inputtime');?></th>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="img175">
                                    ...
                                    <!--div class="sjt"><?php echo getSec2Time($r['duration'])?></div-->
                                    <img src="<?php echo $r['thumb'] ? $r['thumb'] : IMG_PATH . 'admin_img/bfqicon1.jpg';?>" id="thumb_images_<?php echo $r['id']?>" width="100px" />
                                </div>
                            </td>
                            <td valign="top">
                                <div class="iptd">
                                    <input class="tit_ipt" value="<?php echo $r['title'];?>" type="text" id="title_<?php echo $r['id'];?>" />
                            ...
                            <td align='center'>
                                <?php echo getNickname($r['username'])?>
                            </td>
                            <td align='center'><?php echo format::date($r['updatetime'],1);?></td>
                            <td align='center'><?php echo format::date($r['inputtime'],1);?></td>
                            <td align='center'>
                                <a href="?m=content&c=content&a=edit&catid=<?php echo $catid;?>&id=<?php echo $r['id']?>"><?php echo L('edit');?></a> |
                                <a href="<?php echo ($r['url']) ? $r['url'] : url('/show/' . $r['catid'] . '/' . $r['id'] . '.html');?>" target="_blank">预览</a>
                            </td>
                        </tr>
                    <?php }
                }
            ...
            <div class="content">
                <div class="btn-box">
                    <label class="btn" for="check_box"><?php echo L('selected_all');?>/<?php echo L('cancel');?></label>
                        ...
                        <?php if($workflow_menu) { ?><input type="button" class="button" value="<?php echo L('reject');?>" onclick="reject_check()"/>
                            <div id='reject_content' style='background-color: #fff;border:#006699 solid 1px;position:absolute;z-index:10;padding:1px;display:none;'>
                                <table cellpadding='0' cellspacing='1' border='0'>
                                    <tr>
                                        <td colspan='2'>
                                            <textarea name='reject_c' id='reject_c' style='width:300px;height:46px;'  onfocus="if(this.value == this.defaultValue) this.value = ''" onblur="if(this.value.replace(' ','') == '') this.value = this.defaultValue;"><?php echo L('reject_msg');?></textarea>
                                        </td>
                                        <td>
                                            <input type='button' value=' <?php echo L('submit');?> ' class="button" onclick="reject_check(1)">
                                        </td>
                                    </tr>
                                </table>
                            </div>
...
<script type="text/javascript">
    ...
    function update_param(fid, obj) {
        ...
        $.get('<?php echo getExtendsUrl()?>', {m:'content', c:'content', a:'update_param', field:field, id:id, value:value, modelid:modelid}, function (data){
            if (data!=200){
                $(obj).val(old_data);
            }
        })
    ...
    function upthumb(fid) {
        window.top.art.dialog({title:'更换缩略图',id:'update_thumb',iframe:'?m=attachment&c=attachments&a=swfupload&args=<?php echo $args;?>&module=content&catid=<?php echo $catid;?>&authkey=<?php echo $authkey;?>',width:'500',height:'420'}, function(){ var d = window.top.art.dialog({id:'update_thumb'}).data.iframe;var did = d.document.getElementById('att-status');var content = $(did).html();
            var content_arr = new Array(); content_arr = content.split('|');var value = content_arr[1];if (value){var thumb_old = $('#thumb_images_'+fid).attr('src');$.ajaxSettings.async = false;$.get('<?php echo getExtendsUrl()?>', {m:'content', c:'content', a:'update_param', field:'thumb', id:fid, value:value, modelid:'<?php echo $modelid;?>'}, function (data){
                if (data==200){
