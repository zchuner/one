...
<script type="text/javascript">
    ...
    if(window.top.$("#current_pos").data('clicknum') == 1 || window.top.$("#current_pos").data('clicknum') == null) {
        parent.document.getElementById('display_center_id').style.display = 'block';
        parent.document.getElementById('main').style.left = '24%';
        parent.document.getElementById('main').style.width = '76%';
        parent.document.getElementById('center_frame').src = '<?php echo url('admin/extend?m=content&c=content&a=public_categorys&type=add&menuid=' . $_GET['menuid'])?>';
                ...
                $.getJSON('?m=admin&c=category&a=public_ajax_search', {catname: value}, function (data) {
                    if (data != null) {
                        ...
                        $.each(data, function (i, n) {
                            if (n.type == '0') {
                                str += '<li><a href="?m=content&c=content&a=init&menuid=822&catid=' + n.catid + '">' + n.catname + '</a></li>';
                            } else {
                                str += '<li><a href="?m=content&c=content&a=add&menuid=822&catid=' + n.catid + '">' + n.catname + '</a></li>';
                            }
                        });
                        $('#search_div').html(str);
                        $('#search_div').show();
