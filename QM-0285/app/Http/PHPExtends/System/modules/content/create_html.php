...
class create_html extends admin
{
    ...
    public function update_urls()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $this->url = extendLoad::load_app_class('url');
            ...
            if ($modelid) {
                ...
                if (!isset($first) && is_array($catids) && $catids[0] > 0) {
                    setcache('url_show_' . session('admin')['userid'], $catids, 'content');
                    $catids = implode(',', $catids);
                    $where .= " AND catid IN($catids) ";
                    $first = 1;
                } elseif ($first) {
                    $catids = getcache('url_show_' . session('admin')['userid'], 'content');
                    $catids = implode(',', $catids);
                    $where .= " AND catid IN($catids) ";
                } else {
                    $first = 0;
                ...
                if ($pages > $page) {
                    ...
                    delcache('url_show_' . session('admin')['userid'], 'content');
                ...
                if (!isset($set_catid)) {
                    ...
                    setcache('update_url_catid' . '-' . $this->siteid . '-' . session('admin')['userid'], $update_url_catids, 'content');
                    $message = L('start_update_urls');
                    $forward = "?m=content&c=create_html&a=update_urls&set_catid=1&pagesize=$pagesize&dosubmit=1";
                    showmessage($message, $forward, 200);
                }
                $catid_arr = getcache('update_url_catid' . '-' . $this->siteid . '-' . session('admin')['userid'], 'content');
    ...
    public function show()
    {
        if (isset($_POST['dosubmit'])) {
            extract($_POST, EXTR_SKIP);
            $this->html = extendLoad::load_app_class('html');
            ...
            if ($modelid) {
                ...
                if (!isset($first) && is_array($catids) && $catids[0] > 0) {
                    setcache('html_show_' . session('admin')['userid'], $catids, 'content');
                    $catids = implode(',', $catids);
                    $where .= " AND catid IN($catids) ";
                    $first = 1;
                    ...
                    $catids = getcache('html_show_' . session('admin')['userid'], 'content');
                    $catids = implode(',', $catids);
                    $where .= " AND catid IN($catids) ";
                } else {
                    $first = 0;
                ...
                if ($pages > $page) {
                    ...
                    delcache('html_show_' . session('admin')['userid'], 'content');
                    $message = L('create_update_success');
                    $forward = '?m=content&c=create_html&a=show';
                }
                showmessage($message, $forward, 200);
                ...
                if (!isset($set_catid)) {
                    ...
                    setcache('update_html_catid' . '-' . $this->siteid . '-' . session('admin')['userid'], $update_url_catids, 'content');
                ...
                $catid_arr = getcache('update_html_catid' . '-' . $this->siteid . '-' . session('admin')['userid'], 'content');
                $autoid = $autoid ? intval($autoid) : 0;
