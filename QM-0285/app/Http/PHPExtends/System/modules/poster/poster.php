...
class poster extends admin
{
    ...
    public function add()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if ($id) {
                ...
                showmessage(L('add_ads_success'), '?m=poster&c=space&a=init');
            } else {
                showmessage(L('operation_failure'), '?m=poster&c=space&a=init');
            }
        } else {
    ...
    public function edit()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if (extendLoad::load_config('system', 'attachment_stat')) {
                $this->attachment_db = extendLoad::load_model('attachment_model');
                $this->attachment_db->api_update('poster', 'poster-' . $_GET['id'], 1);
            }
            showmessage('修改成功', '?m=poster&c=poster&a=init&spaceid=' . $_GET['spaceid']);
        } else {
    ...
    public function stat()
    {
         ...
         */
        $sdb = extendLoad::load_model('poster_stat_model'); //调用广告统计的数据模型
        ...
        if (is_numeric($_GET['click'])) {
            ...
            $getGroup = safe_replace(trim($_GET['group']));

            //如果设置了按地区或者按ip分类
            if ($getGroup) {
                $group = " `" . $getGroup . "`";
                $fields = "*, COUNT(" . $getGroup . ") AS num";
                $order = " `num` DESC";
            }
