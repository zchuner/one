...
var str = "<div id='PCMSAD_"+this.PosID+"'><ul>";
        var AD = eval('('+content+')');
        var count = 0;
        if(AD.ADText.length){
        ...
        str += "<li><a href="+this.URL+"poster_click?siteid="+this.SiteID+"&id="+AD.ADText[i].textID+"&url="+AD.ADText[i].textLinkUrl+" target='_blank' title='"+AD.ADText[i].textContent+"'>"+AD.ADText[i].textContent+"</a></li>";
        }
        str += "</ul></div>";
document.write(str);
}
...
cmsAD_{$pinfo[0]['id']}.URL = "{APP_PATH}poster/index/";
